...
(def preset-circle
  {:id :circle
   :name "Circle"
   :category :geometric
   :parameters (into (color-params "Color" [1.0 1.0 1.0])
                     [(float-param :radius "Radius" 0.5 0.1 1.0)
...
(def preset-square
  {:id :square
   ...
   :category :geometric
   :parameters (into (color-params "Color" [1.0 1.0 1.0])
                     [(float-param :size "Size" 0.5 0.1 1.0)
...
(def preset-star
  {:id :star
   ...
   :category :geometric
   :parameters (into (color-params "Color" [1.0 1.0 1.0])
                     [(int-param :spikes "Spikes" 5 3 12)
...
(def preset-wave
  {:id :wave
   ...
   :category :wave
   :parameters (into (color-params "Color" [1.0 1.0 1.0])
                     [(float-param :amplitude "Amplitude" 0.3 0.1 0.8)
...
(def preset-beam-fan
  {:id :beam-fan
   ...
   :category :beam
   :parameters (into (color-params "Color" [1.0 1.0 1.0])
                     [(int-param :num-points "Points" 8 2 32)
...
(def preset-horizontal-line
  {:id :horizontal-line
   ...
   :category :beam
   :parameters (into (color-params "Color" [1.0 1.0 1.0])
                     [(float-param :length "Length" 1.0 0.1 2.0)
                      (int-param :num-points "Point Count" 64 3 256)])
...
(defn get-default-params
  ...
  [preset-id]
  (when-let [preset (presets-by-id preset-id)]
