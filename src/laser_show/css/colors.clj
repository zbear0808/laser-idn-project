...
;; ============================================================================
;; Inline Style Color Accessors
;; ============================================================================
;;
;; These functions provide easy access to theme colors for inline :style usage.
;; They eliminate the need for hardcoded hex values in view files.
;;
;; Usage in views:
;;   :style (str "-fx-text-fill: " (css/text-muted) ";")
;;   :style (str "-fx-background-color: " (css/bg-base) ";")

;; Require theme lazily to avoid circular deps
(defn- get-base-colors []
  (require 'laser-show.css.theme)
  @(resolve 'laser-show.css.theme/base-colors))

(defn- get-semantic-colors []
  (require 'laser-show.css.theme)
  @(resolve 'laser-show.css.theme/semantic-colors))

;; Gray levels
(defn bg-base
  "Main background color (#121212)"
  []
  (:bg-base (get-base-colors)))

(defn interactive
  "Interactive element background (#3D3D3D)"
  []
  (:interactive (get-base-colors)))

(defn border
  "Border and hover state color (#505050)"
  []
  (:border (get-base-colors)))

(defn text-muted
  "Muted/secondary text color (#808080)"
  []
  (:text-muted (get-base-colors)))

;; Accent colors
(defn green
  "Success/action button color (#4CAF50)"
  []
  (:green (get-base-colors)))

(defn blue
  "Selection/active state color (#2196F3)"
  []
  (:blue (get-base-colors)))

(defn orange
  "Warning color (#FF9800)"
  []
  (:orange (get-base-colors)))

(defn red
  "Danger/error color (#fd3636)"
  []
  (:red (get-base-colors)))

;; Text colors from semantic
(defn text-primary
  "Primary text color (white)"
  []
  (:text-primary (get-semantic-colors)))

(defn text-secondary
  "Secondary text color (#B0B0B0)"
  []
  (:text-secondary (get-semantic-colors)))
