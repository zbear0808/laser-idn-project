...
(defn saturate
  "Increase the saturation of a color (make it more vivid).
  ...
  (let [rgb (parse-hex hex-str)
        hsl (rgb->hsl rgb)
        new-s (min 1.0 (+ (:s hsl) (* amount (- 1.0 (:s hsl)))))
        new-rgb (hsl->rgb (assoc hsl :s new-s))]
    (to-hex (assoc new-rgb :a (:a rgb)))))


;; =============================================================================
;; Convenience Functions for UI States
;; =============================================================================

(defn hover-color
  "Generate a hover state color (slightly lighter).
   Default: 10% lighter"
  ([hex-str] (hover-color hex-str 0.10))
  ([hex-str amount] (lighten hex-str amount)))

(defn active-color
  "Generate an active/pressed state color (slightly darker).
   Default: 10% darker"
  ([hex-str] (active-color hex-str 0.10))
  ([hex-str amount] (darken hex-str amount)))

(defn disabled-color
  "Generate a disabled state color (desaturated and with reduced opacity).
   Default: 50% desaturated, 60% opacity"
  ([hex-str] (disabled-color hex-str 0.5 0.6))
  ([hex-str desat-amount alpha]
   (-> hex-str
       (desaturate desat-amount)
       (with-alpha alpha))))

(defn focus-color
  "Generate a focus ring color (slightly lightened).
   Default: 15% lighter"
  ([hex-str] (focus-color hex-str 0.15))
  ([hex-str amount] (lighten hex-str amount)))
...
(defn mix
  "Mix two colors together.
   ...
   Parameters:
     ...
     weight - Weight of color1, 0.0-1.0 (0.5 = equal mix)
   
   Returns: Mixed hex color string"
  [color1 color2 weight]
  (let [rgb1 (parse-hex color1)
        rgb2 (parse-hex color2)
        w (max 0.0 (min 1.0 weight))
        mix-channel (fn [c1 c2] (int (+ (* c1 w) (* c2 (- 1.0 w)))))]
    (to-hex {:r (mix-channel (:r rgb1) (:r rgb2))
             :g (mix-channel (:g rgb1) (:g rgb2))
             :b (mix-channel (:b rgb1) (:b rgb2))
             :a (mix-channel (:a rgb1) (:a rgb2))})))
...
(defn luminance
  "Calculate the relative luminance of a color (0.0 - 1.0).
   Used for calculating contrast ratios."
  [hex-str]
  (let [{:keys [r g b]} (parse-hex hex-str)
        to-linear (fn [c]
                    (let [c (/ c 255.0)]
                      (if (<= c 0.03928)
                        (/ c 12.92)
...
(defn readable-text-color
  ...
  ([bg-color] (readable-text-color bg-color "#FFFFFF" "#000000"))
  ([bg-color light-text dark-text]
   (if (dark-color? bg-color) light-text dark-text)))
