(ns laser-show.views.components.visual-editors.oklab-hue-canvas
  "Horizontal gradient canvas for Oklab hue selection (0-360°).
   
   Features:
   - Horizontal hue gradient strip from 0° to 360° in Oklab space
...
(defn- draw-oklab-hue-gradient!
  "Draw a horizontal hue gradient on the canvas using Oklab color space.
   The gradient covers the full 0 to 360 degree hue range with constant L and C."
...
(defn oklab-hue-canvas
  "Horizontal gradient canvas for Oklab hue selection.
  ...
  {:fx/type fx/ext-on-instance-lifecycle
   ...
   (fn [^Canvas canvas]
     (let [gc (.getGraphicsContext2D canvas)
           ...
           ;; Render function
           render! (fn []
                     (draw-oklab-hue-gradient! canvas hue-strip-width hue-strip-height @hue-atom))
