(ns laser-show.views.components.visual-editors.oklab-hue-shift-canvas
  "Two-strip hue transformation canvas for Oklab hue shift visualization.
   
   Features:
   - Top strip: Static input hue gradient (0° to 360°) in Oklab
   - Bottom strip: Shifted output hue gradient in Oklab
   ...
   Usage:
   {:fx/type oklab-hue-shift-canvas
    :fx/key [unique-id]
    :degrees 45.0
...
(def ^:const fixed-lightness 0.70)
(def ^:const fixed-chroma 0.16)

(defn- draw-oklab-hue-shift-strips!
  "Draw two hue strips showing input→output transformation in Oklab space.
  ...
  (let [gc (.getGraphicsContext2D canvas)
    ...
    (doseq [x (range (int strip-width))]
      (let [hue (* (/ (double x) strip-width) 360.0)
            [L a b-ok] (colors/oklch->oklab fixed-lightness fixed-chroma hue)
            [r g b] (colors/oklab->rgb L a b-ok)]
        (.setFill gc (Color/color (u/clamp r 0.0 1.0)
                                  (u/clamp g 0.0 1.0)
                                  (u/clamp b 0.0 1.0)
                                  1.0))
    ...
    (doseq [x (range (int strip-width))]
      (let [input-hue (* (/ (double x) strip-width) 360.0)
            ...
            [L a b-ok] (colors/oklch->oklab fixed-lightness fixed-chroma output-hue)
            [r g b] (colors/oklab->rgb L a b-ok)]
        (.setFill gc (Color/color (u/clamp r 0.0 1.0)
                                  (u/clamp g 0.0 1.0)
                                  (u/clamp b 0.0 1.0)
  ...
  {:fx/type fx/ext-on-instance-lifecycle
   ...
   (fn [^Canvas canvas]
     (let [gc (.getGraphicsContext2D canvas)
           ...
           ;; Render function
           render! (fn []
                     (draw-oklab-hue-shift-strips! canvas hue-shift-strip-width hue-shift-strip-height @degrees-atom))
