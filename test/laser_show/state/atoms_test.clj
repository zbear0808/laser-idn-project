...
(deftest test-effects-operations
  (testing "Initially no effects cells"
    (is (empty? (state/get-effects-cells))))
  
  (testing "Set effect cell (chain format)"
    (state/set-effect-cell! 0 0 {:effects [{:effect-id :scale :params {:x 1.5}}
                                            {:effect-id :rotate :params {:angle 90}}]
                                  :active true})
    (let [cell (state/get-effect-cell 0 0)]
      (is (= true (:active cell)))
      (is (= 2 (count (:effects cell))))
      (is (= :scale (:effect-id (first (:effects cell)))))
      (is (= :rotate (:effect-id (second (:effects cell)))))))
  
  (testing "Clear effect cell"
    (state/set-effect-cell! 1 1 {:effects [{:effect-id :test :params {}}] :active false})
    (state/clear-effect-cell! 1 1)
    (is (nil? (state/get-effect-cell 1 1))))
  
  (testing "Toggle effect cell active"
    (state/set-effect-cell! 2 0 {:effects [{:effect-id :scale :params {:x 1.0}}] :active false})
    (is (true? (state/toggle-effect-cell-active! 2 0)))
    (is (true? (:active (state/get-effect-cell 2 0))))
    (is (false? (state/toggle-effect-cell-active! 2 0)))
    (is (false? (:active (state/get-effect-cell 2 0)))))
  
  (testing "Get all active effect instances in row-major order"
    (state/reset-effects!)
  ...
  (testing "Chain operations - add effect"
    (state/reset-effects!)
    (state/add-effect-to-cell! 0 0 {:effect-id :scale :params {:x 1.5 :y 1.5}})
    (is (= 1 (state/get-effect-count 0 0)))
    (is (true? (state/cell-has-effects? 0 0)))
    
    (state/add-effect-to-cell! 0 0 {:effect-id :rotate :params {:angle 45}})
    (is (= 2 (state/get-effect-count 0 0)))
    (let [cell (state/get-effect-cell 0 0)]
      (is (= :scale (:effect-id (first (:effects cell)))))
      (is (= :rotate (:effect-id (second (:effects cell)))))))
  
  (testing "Chain operations - remove effect"
    (state/reset-effects!)
